...
//u8 tcp_client_sendbuf[64]="ENC28J60 NETCONN TCP Client send data\r\n";	//TCP客户端发送数据缓冲区
u8 tcp_client_flag;		//TCP客户端数据发送标志位

//TCP客户端任务
#define TCPCLIENT_PRIO		7
//任务堆栈大小
...
static void tcp_client_thread(void *arg)
{
	...
	while (1) 
	{
		...
		netconn_bind(tcp_clientconn,NULL,7000);
		...
		{ 
			...
			while(1)
			{
				...
				{
/*
					err = netconn_write(tcp_clientconn ,GPRS_Data.Sensor_Data,(GPRS_Data.Sensor_Data[2] + 3),NETCONN_COPY); //发送tcp_server_sentbuf中的数据
					if(err != ERR_OK)
					{
						printf("发送失败\r\n");
					}

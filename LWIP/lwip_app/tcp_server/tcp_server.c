		...
		{ 
			...
			while(1)
			{
				...
				{		
					...
					if (1) {
						u8 ch = tcp_server_recvbuf[0] - 0x30;
						u8 val = tcp_server_recvbuf[1] - 0x30;
						SetGPIO(ch, val);
						printf("set %d to %d\r\n", ch, val);
					}
...
INT8U tcp_server_init(void)
{
	...
	OS_ENTER_CRITICAL();	//关中断
	res = OSTaskCreate(tcp_server_thread,
						(void*)0,
						(OS_STK*)&TCPSERVER_TASK_STK[TCPSERVER_STK_SIZE-1],
						TCPSERVER_PRIO); //创建TCP服务器线程

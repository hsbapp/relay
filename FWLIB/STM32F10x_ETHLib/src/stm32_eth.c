...
uint32_t ETH_Init(ETH_InitTypeDef* ETH_InitStruct, uint16_t PHYAddress)
{
  ...
  __IO uint32_t i = 0;
	uint16_t regester1 = 0, regester2 = 0;
...
//  }
  ...
  if(ETH_InitStruct->ETH_AutoNegotiation != ETH_AutoNegotiation_Disable)
  {
    ...
    {
      timeout++;
			regester1 = ETH_ReadPHYRegister(PHYAddress, PHY_BSR);
			printf("t2: %x\n", regester1);
    } while (!(regester1 & PHY_Linked_Status) && (timeout < PHY_READ_TO));
    ...
    if(!(ETH_WritePHYRegister(PHYAddress, PHY_BCR, PHY_AutoNegotiation)))  //开启自动协商功能
    {
      /* Return ERROR in case of write timeout */
      err = ETH_ERROR;
	  printf("t4\n");
...
/////////////////////////////////////////////////////			
			regester2 = ETH_ReadPHYRegister(PHYAddress, 0x00);
//			printf("register 0x00 : %x\r\n",regester2);
			timeout = 0;
			
			/* Enable 100M Speed */
			if(!(ETH_WritePHYRegister(PHYAddress, PHY_BCR, regester2 | PHY_FULLDUPLEX_100M)))  //开启自动协商功能
			{
				/* Return ERROR in case of write timeout */
				err = ETH_ERROR;
			}
//			    /* Wait until the 100M Speed  will be completed */
			do
			{
				timeout++;
//				printf("PHY Read Register ERROR\r\n");
			} while (!(ETH_ReadPHYRegister(PHYAddress, PHY_BSR) & PHY_FULLDUPLEX_100M) && (timeout < (uint32_t)PHY_READ_TO));
			//////////////////////////////////////////////////////////
    }
  }
  else
  {
    while(!ETH_WritePHYRegister(PHYAddress, PHY_BCR,  0x3100) == 0)
    {
      /* Return ERROR in case of write timeout */
      err = ETH_ERROR;
			printf("PHY write Register ERROR\r\n");
    }
		regester1 = ETH_ReadPHYRegister(0x00, 0x00);
		printf("register 0x00 : %x\r\n",regester1);
...
uint32_t ETH_Init1(ETH_InitTypeDef* ETH_InitStruct, uint16_t PHYAddress)
{
  uint32_t RegValue = 0, tmpreg = 0;
  __IO uint32_t i = 0;
  RCC_ClocksTypeDef  rcc_clocks;
  ...
  {  
    ...
    if((RegValue & PHY_SPEED_STATUS) != (uint32_t)RESET)
    {
...
void ETH_DMATxDescChainInit1(ETH_DMADESCTypeDef *DMATxDescTab, uint8_t* TxBuff, uint32_t TxBuffCount)
{
  uint32_t i = 0;
  ETH_DMADESCTypeDef *DMATxDesc;
